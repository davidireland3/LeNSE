...
if __name__ == "__main__":
    ...
    cuda = True
    args = sys.argv[1:]
    opts, args = getopt.getopt(args, "g:n:c:b:a:t:f:C:E:")
    for opt, arg in opts:
        if opt in ['-g']:
    ...
    with open(f"{train_graph_name}/budget_{soln_budget}/{encoder_name}/trained_dqn", mode="rb") as f:
        dqn = pickle.load(f)
    dqn.epsilon = 0.01
    env = TestEnv(graph, soln_budget, subgraph_size, encoder, test_graph_name, chunksize=chunksize, action_limit=action_limit, beta=50, cuda=cuda)
    ratios = []
    rewards = []
    num_nodes = []
    num_edges = []

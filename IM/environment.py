...
from functions import expected_spread, relabel_graph, make_graph_features_for_encoder, relabel_edgelist
import random
from IMM import imm
...
class BaseEnvironment:
    ...
    def __init__(self, graph, solution_budget, subgraph_size, encoder, graph_name, Type="main", model="IC", chunksize=80, action_limit=5000, cuda=False):
    ...
    def get_optimal_scores(self):
        try:
            ...
            start = time.time()
            seeds, _ = imm(self.graph, self.solution_budget, self.model)
            end = time.time()
            with open(f"{self.graph_name}/budget_{self.solution_budget}/time_taken_to_get_seeds", mode='w') as f:
                f.write(f"It took {(end - start) / 60} minutes to get a solution.")
            score = expected_spread(seeds, self.graph_name, self.Type, self.model, pool=self.pool, chunksize=self.chunksize)
            with open(f"{self.graph_name}/budget_{self.solution_budget}/score_and_seeds", mode="wb") as f:
                pickle.dump((seeds, score, None), f)
            self.optimal_score = score

    def run_imm_on_final_subgraph(self):
        graph, transformation = relabel_graph(self.state, True)
        seeds, _ = imm(graph, self.solution_budget, self.model)
...
class GraphModification(BaseEnvironment):
    def __init__(self, graph, solution_budget, subgraph_size, encoder, best_embedding, graph_name, Type="main", model="IC", chunksize=80, action_limit=5000, beta=1, cuda=False):

        super(GraphModification, self).__init__(graph, solution_budget, subgraph_size, encoder, graph_name, Type, model, chunksize, action_limit, cuda=cuda)
    ...
    def step(self, action):
        ...
        if done:
            ...
            score = expected_spread(seeds, self.graph_name, self.Type, pool=self.pool, chunksize=self.chunksize)
...
class TestEnv(GraphModification):
    def __init__(self, graph, solution_budget, subgraph_size, encoder, graph_name, Type="main", model="IC", chunksize=80, action_limit=1000, beta=1, cuda=False):

        super(TestEnv, self).__init__(graph, solution_budget, subgraph_size, encoder, None, graph_name, Type, model, chunksize, action_limit, beta, cuda)
    ...
    def get_optimal_scores(self):
        for budget in self.budgets:
            try:
                ...
                _, seeds = imm(self.graph, budget)
                score = expected_spread(seeds, self.graph_name, chunksize=self.chunksize, pool=self.pool)
...
class GuidedExplorationEnv(GraphModification):
    ...
    def get_optimal_scores(self):
        try:
            ...
            _, seeds = imm(self.graph, self.solution_budget)
            score = expected_spread(seeds, self.graph_name, pool=self.pool, chunksize=self.chunksize)
